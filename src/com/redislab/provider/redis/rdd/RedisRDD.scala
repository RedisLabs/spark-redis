...
class RedisListRDD(sc: SparkContext,
  ...
  def getSET(nodes: Array[(String, Int, Int, Int, Int, Int)], keys: util.ArrayList[String]): Iterator[String] = {
    mapByNode(nodes, keys).flatMap{
      x => {
        val jedis = new Jedis(x._1._1, x._1._2)
        val pipeline = jedis.pipelined
  ...
  def getLIST(nodes: Array[(String, Int, Int, Int, Int, Int)], keys: util.ArrayList[String]): Iterator[String] = {
    mapByNode(nodes, keys).flatMap{
      x => {
        val jedis = new Jedis(x._1._1, x._1._2)
        val pipeline = jedis.pipelined
