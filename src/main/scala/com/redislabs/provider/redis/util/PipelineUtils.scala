...
object PipelineUtils {
  ...
  /**
    ...
    */
  def mapWithPipeline[A](conn: Jedis, maxPipelineSize: Int, items: TraversableOnce[A])(f: (Pipeline, A) => Unit): Seq[AnyRef] = {
  ...
  def mapWithPipeline[A](conn: Jedis, items: TraversableOnce[A])(f: (Pipeline, A) => Unit)
    ...
    mapWithPipeline(conn, readWriteConfig.maxPipelineSize, items)(f)
  }
  ...
  def foreachWithPipeline[A](conn: Jedis, items: TraversableOnce[A])(f: (Pipeline, A) => Unit)
    ...
    foreachWithPipeline(conn, readWriteConfig.maxPipelineSize, items)(f)
  }
  ...
  /**
    ...
    */
  def foreachWithPipelineNoLastSync[A](conn: Jedis, maxPipelineSize: Int, items: TraversableOnce[A])(f: (Pipeline, A) => Unit): Pipeline = {
    // iterate over items and create new pipelines periodically
    var i = 0
    var pipeline = conn.pipelined()
    for (x <- items) {
  ...
  def foreachWithPipelineNoLastSync[A](conn: Jedis, items: TraversableOnce[A])(f: (Pipeline, A) => Unit)
    ...
    foreachWithPipelineNoLastSync(conn, readWriteConfig.maxPipelineSize, items)(f)
  }
